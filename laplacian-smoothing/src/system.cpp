...
void MySystem::handleAssetLoading( Ra::Engine::Entity* entity,
    ...
    LOG( logINFO ) << "New entity: " << qPrintable( entityName ) << " with " << components.size()
                   << " geometry" << ( components.size() <= 1 ? " component." : " components." );

    // Register the geometry data associated to the loaded entities
    for ( const auto& comp : components )
    {
        emit newInputModelRegistered( entityName, QString::fromStdString( comp->getName() ) );
    }
}
...
void MySystem::compute( Param p ) {
    ...
    Mesh* mesh = dynamic_cast<Mesh*>( Ra::Engine::RadiumEngine::getInstance()->getMesh(
        p.entityName.toStdString(), p.dataId.toStdString() ) );
    ...
    if ( mesh != nullptr )
    {
        ...
        for ( e_it = topologicalMesh.edges_begin(); e_it != e_end; ++e_it )
        {
            ...
            d1 = ( *p1 - *p2 );
            d1.normalize();
            weight += static_cast<TopologicalMesh::Scalar>( 1.0 ) /
        ...
        for ( v_it = topologicalMesh.vertices_begin(); v_it != v_end; ++v_it )
        {
            ...
            if ( weight )
                topologicalMesh.property( vertex_weights_, *v_it ) =
                    static_cast<TopologicalMesh::Scalar>( 1.0 ) / weight;
        }
        ...
        for ( v_it = topologicalMesh.vertices_begin(), cog_it = cogs.begin(); v_it != v_end;
              ++v_it, ++cog_it )
        {
